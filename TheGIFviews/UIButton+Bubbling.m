...
- (UIColor *)getRandomColor{
    NSArray *colors = @[ HEX_COLOR(0xFC6782), HEX_COLOR(0xFFEE58), HEX_COLOR(0x94EFDF),
...
- (void)startAnimationHeartLayer:(CALayer *)heartLayer{
    ...
    CGFloat destination_y = heartLayer.position.y/2;
    CGPoint destination = CGPointMake(destination_x, destination_y);
    [posAnimation setToValue:[NSValue valueWithCGPoint:destination]];
    //透明度动画
    CAKeyframeAnimation *alphaAnimation = [CAKeyframeAnimation animationWithKeyPath:@"opacity"];
    alphaAnimation.values = @[@1.0,@1.0,@0.0];
    alphaAnimation.keyTimes = @[@0.0,@0.9,@1.0];
    //旋转动画
    CABasicAnimation *angleAnimation = [CABasicAnimation animationWithKeyPath:@"transform"];
    [angleAnimation setFromValue:[NSValue valueWithCATransform3D:CATransform3DMakeScale(1.0f, 1.0f, 1.0f)]];
    CATransform3D destTrans = CATransform3DMakeRotation(M_PI/4, 0, 0, (arc4random() %2000)/1000.0f - 1);
    ...
    [groupAnimation setValue:heartLayer forKey:LAYER_ANIMATION_LAYER_NAME];
